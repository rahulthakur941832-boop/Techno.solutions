...
const WP_URL = 'https://annual-albus-tr.zipwp.top/about/';
...
async function main() {
  ...
  try {
    console.log(`Fetching authenticated About page at ${WP_URL}...`);
    const res = await fetch(WP_URL, { headers });
    if (!res.ok) {
      throw new Error(`HTTP Error: ${res.status} ${res.statusText}`);
    }
    const html = await res.text();
    console.log(`Successfully fetched authenticated About page. Length: ${html.length} bytes.`);
