...
async function main() {
  ...
  try {
    console.log(`Fetching authenticated homepage at ${WP_URL}...`);
    const res = await fetch(WP_URL, { headers });
    if (!res.ok) {
      throw new Error(`HTTP Error: ${res.status} ${res.statusText}`);
    }
    const html = await res.text();
    console.log(`Successfully fetched authenticated homepage. Length: ${html.length} bytes.`);
    ...
    // Search for element ids or sections
    const ids = [...html.matchAll(/id=["']([^"']+)["']/gi)].map(m => m[1]);
    console.log('Found element IDs:', ids.slice(0, 30));
